...
<header id="appHeader">
  ...
  <div class="header-desktop">
    <div class="container d-flex justify-content-between align-items-center">
      <form action="{{ front_route('products.index') }}" method="get" class="search-group" id="searchForm">
        <input type="text" class="form-control" name="keyword" placeholder="{{ __('front/common.search') }}"
          value="{{ request('keyword') }}" id="searchInput">
        <button type="submit" class="btn" id="toggleButton"><i class="bi bi-search"></i></button>
      </form>
      <div class="left">
        <h1 class="logo">
          <a href="{{ front_route('home.index') }}">
            <img src="{{ theme_asset('erasmart', 'images/logo1.png') }}" class="img-fluid">
          </a>
        </h1>
      </div>
      <div class="right">
