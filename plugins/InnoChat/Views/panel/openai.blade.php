...
@push('header')
  ...
  <script src="{{ plugin_asset('inno_chat', 'js/highlight/highlight.min.js') }}"></script>
  <link rel="stylesheet" type="text/css" href="{{ plugin_asset('inno_chat', 'js/highlight/atom-one-dark.min.css') }}">
@endpush
    ...
    <div class="card-body">
      <div class="col-12 answer-wrap">
        <div class="border p-3 bg-white" id="answer">
          <div class="not-answer"><i class="bi bi-activity"></i> {{ __('InnoChat::common.no_question') }}</div>
        </div>
        <div class="input-group mb-3 mt-4">
          <input type="text" id="ai-input" class="form-control rounded-0 form-control-lg"
                 ...
                 {{ $error ? 'disabled' : '' }} aria-label="{{ __('InnoChat::common.enter_question') }}"
                 aria-describedby="button-addon2">
          <button class="btn btn-primary px-4 rounded-0" {{ $error ? 'disabled' : '' }} type="button" id="ai-submit"><i
      ...
      if ($(this).scrollTop() == 0) {
        if (current_page < last_page) {
          if (!$('.text-loading').length) {
            $('#answer').prepend(
              '<div class="text-center py-3 text-secondary text-loading"><i class="bi bi-activity"></i> {{ __('InnoChat::common.loading') }}</div>'
            );
          }
        ...
        if (current_page == last_page) {
          if (!$('.text-loading').length) {
            $('#answer').prepend(
              '<div class="text-center py-3 text-secondary text-loading"><i class="bi bi-activity"></i> {{ __('InnoChat::common.no_more') }}</div>'
            );
          }
        }
      }
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            html += '<div class="answer-list">',
              ...
              html += '<div class="d-flex mb-2"><div class="text-secondary">{{ __('InnoChat::common.qa_q') }}：</div><div class="w-100">' + question + '</div></div>',
              html += '<div class="d-flex"><div class="text-secondary">{{ __('InnoChat::common.qa_a') }}：</div><div class="w-100">' + answer + '</div></div>'
            html += '</div>'
          ...
          if (data.data && data.data.length) {
            ...
            data.data.forEach(function (item, index) {
              html += '<div class="answer-list ' + (!index ? 'first' : '') + '">',
                ...
                html += '<div class="d-flex mb-2"><div class="text-secondary">{{ __('InnoChat::common.qa_q') }}：</div><div class="w-100">' + item.question + '</div></div>',
                html += '<div class="d-flex"><div class="text-secondary">{{ __('InnoChat::common.qa_a') }}：</div><div class="w-100">' + marked.parse(item.answer) + '</div></div>'
              html += '</div>'
            })
