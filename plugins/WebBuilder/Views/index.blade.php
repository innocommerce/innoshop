...
<body class="@yield('body-class')">
    ...
    <div class="card-body d-flex">
      ...
      <div class="module-wrap" :class="{ 'collapsed': isCollapsed }">
        ...
        <div class="module-tabs">
          ...
          <div class="tab-item" :class="{ 'active': showPropertyPanel }" @click="showPropertyPanel = true"
        ...
        <div class="panel-content" v-show="!showPropertyPanel">
          <draggable class="modules-list dragArea list-group"
            ...
            :list="source.modules" :clone="cloneDefaultField">
        ...
        <div class="panel-content" v-show="showPropertyPanel">
          <div v-if="form.modules.length > 0 && design.editingModuleIndex >= 0" class="component-wrap">
            <component v-if="editingModuleComponent" :is="editingModuleComponent" :key="design.editingModuleIndex"
              :module="form.modules[design.editingModuleIndex].content" @on-changed="moduleUpdated">
            </component>
          </div>
        </div>
      </div>
      ...
      <div class="preview-wrap flex-grow-1 mx-3">
        ...
        <div class="preview-container">
          ...
          <div class="editable-area">
            <!-- 预览区域的拖拽配置 -->
            <draggable class="view-modules-list dragArea list-group" :list="form.modules"
              :options="{
                  group: {
                      name: 'people',
                      pull: false,
              ...
              <div v-else v-for="(module, index) in form.modules" :key="index"
                ...
                <div class="module-actions" v-show="design.editingModuleIndex === index">
                  <button class="btn-action" @click.stop="moveModule(index, 'up')" :disabled="index === 0"
                    title="上移">
                ...
                <!-- slideshow 轮播图 -->
                <div
                  v-if="module && module.code == 'slideshow' && module.content && module.content.images && module.content.images.length">
                  <img :src="module.content.images[0].image[source.locale]" class="img-fluid">
                </div>

                <!-- image100 图片模块 -->
                <div
                  v-if="module && module.code == 'image100' && module.content && module.content.images && module.content.images.length">
                  <img :src="module.content.images[0].image[source.locale]" class="img-fluid">
                </div>
                ...
                <div v-if="module && module.code == 'product' && module.content">
                  ...
                  <div v-if="!module.content.products || !module.content.products.length" class="hint-right-edit">
                    请在左侧配置商品
                  </div>
                  <div class="row gx-3 gx-lg-4" v-else>
                    <div class="col-6 col-md-4 col-lg-3" v-for="item in module.content.products"
                      :key="item.id">
                ...
                <div v-if="module && module.code == 'four_image' && module.content">
                  <!-- 添加标题和副标题 -->
                  <div v-if="module.content.title && module.content.title[source.locale]" class="module-title-wrap">
                    <div class="module-title">@{{ module.content.title[source.locale] }}</div>
                  ...
                  <!-- 图片内容 -->
                  <div v-if="module && module.content.images && module.content.images.length" class="four-image-grid">
                    <div class="image-row">
                ...
                <div v-if="module && module.code == 'four_image-plus' && module.content">
                  ...
                  <!-- 图片内容 -->
                  <div v-if="module.content.images && module.content.images.length"
                ...
                <div v-if="module && module.code == 'article' && module.content">
                  ...
                  <div v-if="!module.content.articles || !module.content.articles.length" class="hint-right-edit">
                    请在左侧配置文章
                  </div>
                  <div class="row gx-3 gx-lg-4" v-else>
                    <div class="col-6 col-md-4 col-lg-3" v-for="item in module.content.articles.slice(0, 4)"
                      :key="item.id">
                      <div class="blog-item">
                        <div class="image">
                          <a :href="item.url">
                ...
                <div v-if="module && module.code == 'rich_text' && module.content">
                  ...
                  <div v-if="module.content.content && module.content.content[source.locale]"
                    class="rich-text-content" v-html="module.content.content[source.locale]">
                  </div>
                  <div v-else class="hint-right-edit">
                    请在左侧编辑富文本内容
                  </div>
          ...
          <div class="pb-images-selector" v-for="(item, index) in form.images" :key="index">
            ...
            <div :class="'pb-images-list ' + (item.show ? 'active' : '')">
              <div class="pb-images-top">
                <pb-image-selector v-model="item.image" :aspectRatio="2" :targetWidth="1000"
  ...
  <template id="module-editor-image100-template">
    <div class="image-edit-wrapper">
      ...
      <div class="module-edit-group">
        ...
        <div class="">
          <div class="pb-images-top">
            <pb-image-selector v-model="form.images[0].image" :aspectRatio="2.0833" :targetWidth="1000"
        ...
        <div class="tab-info">
          <div class="module-edit-group">
            <div class="autocomplete-group-wrapper">
              <el-autocomplete class="inline-input" v-model="keyword" value-key="name" size="small"
                :fetch-suggestions="querySearch" placeholder="请输入关键字搜索" :highlight-first-item="true"
                @select="handleSelect"></el-autocomplete>

              <div class="item-group-wrapper" v-loading="loading">
                <template v-if="productData.length">
                  <draggable ghost-class="dragabble-ghost" :list="productData" @change="itemChange"
            ...
            <div class="product-info" v-loading="loading">
              <template v-if="linkDialog.data.length">
                ...
                <ul class="product-list">
                  <li v-for="(product, index) in linkDialog.data"
                    ...
                    <div class="left">
                      <span
                        :class="'checkbox-plus ' + (link.value == product.id ? 'active' : '') + (!product.active ? 'no-status' :
                            '')"></span>
              ...
              <div class="product-info-no" v-if="!linkDialog.data.length && loading === false">
                ...
                <div class="no-text">数据不存在或已被删除, <a :href="linkTypeAdmin"
                    target="_blank">去添加@{{ dialogTitle }}</a></div>
              </div>
            </div>
          </template>
        </div>
  ...
  <template id="text-i18n-template">
    <div class="text-i18n-template">
      <el-tabs v-if="languages.length > 1" value="language-zh_cn" :stretch="languages.length > 5 ? true : false"
        type="card">
        <el-tab-pane v-for="(item, index) in languages" :key="index" :label="item.name"
          :name="'language-' + item.code">
      ...
      <div class="i18n-inner" v-else>
        <el-input :type="type" :rows="4" :placeholder="languages[0].name"
          :size="size" :value="value[languages[0].code]"
          @input="(val) => {valueChanged(val, languages[0].code)}"></el-input>
      </div>
    </div>
  </template>
      ...
      <div class="module-edit-group">
        ...
        <draggable ghost-class="dragabble-ghost" :list="form.images"
          ...
          <div class="pb-images-selector" v-for="(item, index) in form.images" :key="index">
            ...
            <div :class="'pb-images-list ' + (item.show ? 'active' : '')">
              <div class="pb-images-top">
                <pb-image-selector v-model="item.image" :aspectRatio="1" :targetWidth="400"
        ...
        <div class="tab-info">
          <div class="module-edit-group">
            <div class="autocomplete-group-wrapper">
              <el-autocomplete class="inline-input" v-model="keyword" value-key="name" size="small"
                :fetch-suggestions="querySearch" placeholder="请输入关键字搜索" :highlight-first-item="true"
                @select="handleSelect" :disabled="articleData.length >= 4">
              </el-autocomplete>

              <div class="item-group-wrapper" v-loading="loading">
                <template v-if="articleData.length">
                  <draggable ghost-class="dragabble-ghost" :list="articleData" @change="itemChange"
                    :options="{ animation: 330 }">
                    <div v-for="(item, index) in articleData" :key="index" class="item">
                      <div>
                        <i class="el-icon-s-unfold"></i>
                        <span>@{{ item.name }}</span>
                      </div>
                      <i class="el-icon-delete right" @click="removeArticle(index)"></i>
                    </div>
                  </draggable>
                </template>
                <template v-else>
      ...
      <div class="module-edit-group">
        ...
        <ul class="nav nav-tabs" role="tablist">
          <li class="nav-item" v-for="lang in languages" :key="lang.code">
            <button class="nav-link" :class="{ active: lang.code === currentLanguage }" :id="'tab-' + lang.code"
              @click="switchLanguage(lang.code)" type="button" role="tab">
              @{{ lang.name }}
            </button>
          </li>
        </ul>
        <div class="tab-content pt-3">
          <div v-for="lang in languages" :key="lang.code" class="tab-pane fade"
            :class="{ 'show active': lang.code === currentLanguage }" :id="'content-' + lang.code">
