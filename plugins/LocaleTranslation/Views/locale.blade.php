      ...
      <div class="d-flex align-items-center mb-2">
        ...
        <select class="form-select wp-200 plugins-select">
          ...
          @foreach ($plugins as $item)
            <option
                value="{{ $item['code'] }}" {{ $plugin_code == $item['code'] ? 'selected' : '' }}>{{ $item['name'] ?? '' }}</option>
          @endforeach
        </select>
      </div>
      ...
      <div class="locales-box">
        <div class="left">
            ...
            <div class="accordion-item" v-for="tree, index in treeData" :key="index">
              ...
              <div :id="'collapseOne-' + index" class="accordion-collapse collapse">
                <div class="">
                  ...
                  <div class="accordion-item" v-for="folder_2, index_2 in tree.folders" :key="index + '-' + index_2"
                       v-if="tree.folders.length">
                    <h2 class="accordion-header">
                      <button class="accordion-button collapsed py-2" type="button" data-bs-toggle="collapse"
                              :data-bs-target="'#collapseOne-' + index + '-' + index_2" aria-expanded="true"
                    ...
                    <div :id="'collapseOne-' + index + '-' + index_2" class="accordion-collapse collapse">
                      <div class="">
                        ...
                        <ul class="files-wrap list-group list-group-flush" v-if="folder_2.files.length">
                          <li :class="['list-group-item cursor-pointer', activeLocale == file ? 'active' : '']"
                              @click="getLanguageEntry(file)" v-for="file, file_index in folder_2.files"
                              :key="index + '-' + file_index"><i class="bi-file-earmark"></i> @{{ file }}
                          </li>
                        </ul>
                      </div>
                    </div>
                  ...
                  <ul class="files-wrap list-group list-group-flush" v-if="tree.files.length">
                    <li :class="['list-group-item cursor-pointer', activeLocale == file ? 'active' : '']"
                        @click="getLanguageEntry(file)" v-for="file, file_index in tree.files"
                        :key="index + '-' + file_index"><i class="bi-file-earmark"></i> @{{ file }}
                    </li>
                  </ul>
                </div>
          ...
          <ul class="files-wrap list-group mt-2" v-if="files.length">
            <li :class="['list-group-item cursor-pointer', activeLocale == file ? 'active' : '']"
                @click="getLanguageEntry(file)" v-for="file, index in files" :key="index"><i
                  class="bi-file-earmark"></i> @{{ file }}
            </li>
          </ul>
        </div>
          ...
          <div class="table-responsive" id="table-responsive">
            <table class="table table-bordered">
              ...
              <th style="background-color: #eaf6ff">
                <div>{{ __('LocaleTranslation::common.column') }}</div>
                <button class="btn btn-sm opacity-0">1</button>
              </th>
              ...
              <th v-for="value, index in localesValues.locale_codes" :key="index" class="text-center locale-th">
                ...
                <button type="button" @click="translateCol(value)" v-if="isShowTranslateBtn(value)"
              ...
              <tr v-for="item, index in localesValues.base.keys" :key="index">
                ...
                <td style="background-color: #D6BFF1">
                  <el-tooltip v-if="isString(localesValues.base.values[item])" class="item" effect="dark"
                              :content="localesValues.base.values[item]" placement="top-start">
                ...
                <td v-for="value, index_2 in localesValues.extra" :key="index + '-' + index_2">
                  <el-tooltip class="item" effect="dark" :content="value[item]" placement="top-start"
                              v-if="value[item]">
        ...
        <div v-else
             class="w-100 h-min-300 border ms-3 fs-2 text-muted d-flex align-items-center justify-content-center">
    ...
    var app = new Vue({
      ...
      methods: {
        ...
        getLanguageEntry(file) {
          ...
          axios.get('{{ panel_route('locale_translations.values') }}', {
            params: {
              file_path: file,
        ...
        translate(locale, code) {
          axios.post('{{ panel_route('locale_translations.translate') }}', {
            file_path: this.activeLocale,
            targets: [code],
            keys: [locale],
            plugin_code: this.plugin_code
          }).then((res) => {
        ...
        format(value) {
          axios.post('{{ panel_route('locale_translations.format') }}', {
            file_path: this.activeLocale,
            targets: [value],
            plugin_code: this.plugin_code
          }).then((res) => {
