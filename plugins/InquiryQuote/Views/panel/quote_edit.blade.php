      ...
      @foreach ($next_statuses as $status)
        @if (!in_array($status['status'], $excluded))
          <button class="btn btn-primary status ms-2" data-status-code="{{ $status['status'] }}"
                  ...
                  @click="edit('{{ $status['status'] }}')">{{ $status['action'] }}</button>
        @endif
      @endforeach
        ...
        <div class="card-body">
          <table class="table products-table align-middle">
                ...
                <tr data-sku-code="{{ $inquiry['sku_code'] }}">
                  ...
                  <td class="col-lg-2">
                    <input type="number" step="1" name="inquiries[{{ $inquiry['id'] }}][quantity]"
                           value="{{ $inquiry['quantity'] }}" class="form-control quantity"
                           data-inquiry-id="{{ $inquiry['id'] }}">
                  </td>
                  <td class="text-align">
                    <div>
            ...
            <tr>
              ...
              <td class="align-middle"></td>
              <td class="mt-3">
                <button type="button" data-quote-id="{{ $quote['id'] }}" class="btn btn-primary" id="submission">
                  提交
                </button>
              </td>
            </tr>
        ...
        <div class="card-body">
          <div class="col-12 col-md-6 mb-4">
            <div class="checkout-select">
              <textarea class="form-control" rows="4" id="orderComment"
                        placeholder="{{ __('front/checkout.order_comment') }}"></textarea>
            </div>
            <div class="mt-3 d-flex justify-content-end">
              <button class="btn btn-primary" id="submitButton" type="button" data-quote-id="{{ $quote['id'] }}">提交
              </button>
            </div>
          </div>
          <div class="border-top">
            <table class="mt-5  table table-response align-middle">
              ...
              <tbody>
              @foreach ($quote->histories->sortByDesc('id') as $history)
                <tr>
                  <td data-title="State">{{ $history->status_format }}</td>
                  <td data-title="Remark" data-bs-toggle="tooltip" title="{{ $history->comment }}"
