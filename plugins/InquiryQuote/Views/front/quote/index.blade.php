  ...
  <div class="container">
                      ...
                      <div class="addresses-wrap">
                        <div class="shipping-address">
                          ...
                          <span class="cursor-pointer" v-if="!source.addressEdit" @click="addressEdit(true)"><i
                            ...
                            __('front/checkout.create_address') }}</span>
                            </div>
                          </div>
                            ...
                            <div :class="['select-item', current.shipping_address_id == address.id ? 'active' : '']"
                              ...
                              <div class="left">
                                ...
                                <div class="select-title">
                                  <div class="address-name mb-1">@{{ address.name }} @{{
                                    address.phone }} @{{ address.zipcode
                                    }}
                                  </div>
                                  <div class="address-info">@{{ address.address_1 }} @{{
            ...
            <ul class="cart-data-list">
              ...
              <ul class="cart-data-list">
                @foreach ($quote_fees as $item)
                  ...
                  @if($item['code'] === 'shipping')
                      ...
                      <span class="total-amount d-flex justify-content-end align-items-center col-lg-6">
                      <input  id="shipping-item-cost-input" class="form-control fee-input" value="{{ $item['inquiry_amount'] }}">
                   @endif
                  </li>
                @endforeach
              </ul>
              <li class="d-flex justify-content-between">
                @if(plugin_setting('inquiry_quote', 'based_seller'))
                  <button type="button" data-customer_id="{{$quote['customer_id']}}" data-quote-id="{{ $quote['id'] }}"
                          data-quote-based="seller"
                          class="btn btn-primary send-quote">向商家询盘
                  </button>
                @endif
                @if(plugin_setting('inquiry_quote', 'based_salesman'))
                  <button type="button" data-customer_id="{{$quote['customer_id']}}" data-quote-id="{{ $quote['id'] }}"
                          data-quote-based="salesman"
    ...
    @else
      <div class="text-center pm-5">
        ...
        <a class="btn btn-primary btn-lg mt-3" href="{{ front_route('home.index') }}">{{ __('front/cart.continue')
      }}</a>
      </div>
    @endif
  </div>
...
@push('footer')
  <script>
    ...
    $('.btn.send-quote').on('click', function () {
      ...
      const fees = { handling, tax, shipping};
      const inquiries = [];
      ...
      axios.put(`${urls.api_base}/quotes/${quoteId}`, {
        comment, inquiries, fees, shipping_address_id, shipping_method_code, based
      }).then(function (res) {
        inno.msg(res.message);
        window.location.reload();
      }).catch(function (err) {
