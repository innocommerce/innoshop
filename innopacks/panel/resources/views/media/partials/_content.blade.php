@section('content')
  <div id="app">
    <div class="media">
          ...
          <el-col :span="12">
            <el-button-group>
              <el-button type="primary" size="small" @click="uploadFile">
                <el-icon><component :is="'Upload'"></component></el-icon> {{ __('panel/media.upload') }}
              </el-button>
            ...
            <el-button-group v-if="isIframeMode">
              <el-button size="small" :type="isMultiSelectMode ? 'primary' : 'default'" @click="toggleMultiSelectMode">
                <el-icon><component :is="'Check'"></component></el-icon> {{ __('panel/media.multi_select_mode') }}
              </el-button>
              <el-button v-if="isMultiSelectMode" size="small" @click="selectAll">
                <el-icon><component :is="'Finished'"></component></el-icon> {{ __('panel/media.select_all') }}
              </el-button>
              <el-button type="primary" size="small" @click="handleConfirm">{{ __('panel/media.select_submit') }}</el-button>
            </el-button-group>
            <el-tag v-if="mediaStats.loaded" type="info" size="small" effect="plain" class="media-stats-tag">
              <el-icon><component :is="'Files'"></component></el-icon>
              @{{ mediaStats.total_files }} {{ __('panel/media.total_files') }}
              <span style="margin: 0 6px; opacity: .5;">·</span>
              @{{ mediaStats.size_readable }}
            </el-tag>
          </el-col>
          <el-col :span="12" class="file-toolbar-actions">
            <el-button-group>
              <el-button size="small" :disabled="selectedFiles.length !== 1" @click="renameSelectedFile">
                <el-icon><component :is="'Edit'"></component></el-icon> {{ __('panel/media.rename') }}
              </el-button>
              <el-button size="small" :disabled="!selectedFiles.length" @click="deleteFiles">
                <el-icon><component :is="'Delete'"></component></el-icon> {{ __('panel/media.delete') }}
              </el-button>
              <el-button size="small" :disabled="!selectedFiles.length" @click="moveFiles">
                <el-icon><component :is="'Folder'"></component></el-icon> {{ __('panel/media.move_to') }}
              </el-button>
              <el-button size="small" :disabled="!selectedFiles.length" @click="copyFiles">
                <el-icon><component :is="'CopyDocument'"></component></el-icon> {{ __('panel/media.copy_to') }}
              </el-button>
            </el-button-group>
            <el-button size="small" style="margin-right: 10px;" :type="showFolders ? 'primary' : 'default'" @click="toggleShowFolders" title="{{ __('panel/media.show_folders') }}">
              <el-icon><component :is="'Folder'"></component></el-icon>
            </el-button>
            <el-button-group>
              <el-button size="small" :type="sortField === 'created' ? 'primary' : 'default'" @click="toggleSortField('created')" title="{{ __('panel/media.sort_by_time') }}">
                <el-icon><component :is="'Clock'"></component></el-icon>
              </el-button>
              <el-button size="small" :type="sortField === 'name' ? 'primary' : 'default'" @click="toggleSortField('name')" title="{{ __('panel/media.sort_by_name') }}">
                <el-icon><component :is="'Operation'"></component></el-icon>
              </el-button>
              <el-button size="small" @click="toggleSortOrder" :title="sortOrder === 'asc' ? '{{ __("panel/media.sort_asc") }}' : '{{ __("panel/media.sort_desc") }}'">
                <el-icon v-if="sortOrder === 'asc'"><component :is="'SortUp'"></component></el-icon>
                <el-icon v-else><component :is="'SortDown'"></component></el-icon>
              </el-button>
            </el-button-group>
            <el-button size="small" @click="refreshAll" :title="'{{ __("panel/media.refresh") }}'">
      ...
      <el-row :gutter="0">
        ...
        <el-col :xs="16" :sm="18" :md="19" :lg="20"
          :xl="21" class="media-right">
          <div class="fm-top-bar">
            <div class="fm-breadcrumb">
              <el-icon class="fm-crumb-lead"><component :is="'FolderOpened'"></component></el-icon>
              <span class="fm-crumb-trail">
                <span
                  v-for="(item, idx) in breadcrumbItems"
                  :key="item.path"
                  class="fm-crumb-item">
                  <span :class="idx < breadcrumbItems.length - 1 ? 'fm-crumb-link' : 'fm-crumb-current'"
                    @click="navigateToBreadcrumb(item.path)">@{{ item.name }}</span>
                  <el-icon v-if="idx < breadcrumbItems.length - 1" class="fm-crumb-sep"><component :is="'ArrowRight'"></component></el-icon>
                </span>
              </span>
            </div>
          </div>
          <div class="file-list">
            <div class="file-list-content">
              <div v-loading="loading" element-loading-text="{{ __('panel/media.loading') }}">
                  ...
                  <el-col :xs="12" :sm="8" :md="6" :lg="4"
                    ...
                    <div :class="['file-card', selectedFiles.includes(file.id || file.path) ? 'selected' : '']"
                        ...
                        <template v-else>
                            <template v-if="file.mime && file.mime.startsWith('image/')">
                              ...
                              <div class="preview-button"
                                ...
                                <span>{{ __('panel/media.preview') }}</span>
                              </div>
                      ...
                      <div class="file-info">
                        ...
                        <p class="file-type">@{{ file.is_dir ? {!! json_encode(__('panel/media.folder')) !!} : file.mime }}</p>
                      </div>
                    </div>
                  </el-col>
                </el-row>
                ...
                <div class="pagination-container">
                  <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                    ...
                    <template #prev-text><span>{{ __('panel/media.prev') }}</span></template>
                    <template #next-text><span>{{ __('panel/media.next') }}</span></template>
                  </el-pagination>
                </div>
                </template>
                <!-- Empty state -->
                <el-empty v-else :description="'{{ __('panel/media.empty') }}'" :image-size="120"></el-empty>
              </div>
            </div>
          </div>
        </el-col>
    ...
    <el-dialog title="{{ __('panel/media.new_folder_dialog') }}" v-model="folderDialog.visible" width="400px">
      ...
      <template #footer>
        <el-button @click="folderDialog.visible = false">{{ __('panel/media.cancel_btn') }}</el-button>
        <el-button type="primary" @click="submitCreateFolder">{{ __('panel/media.ok_btn') }}</el-button>
      </template>
    </el-dialog>

    <!-- Upload file dialog -->
    <el-dialog title="{{ __('panel/media.upload_dialog') }}" v-model="uploadDialog.visible" width="500px" @open="onUploadDialogOpen">
      <el-upload class="file-uploader" drag multiple :action="uploadUrl" :headers="uploadHeaders"
        ...
        <div class="el-upload__text">{{ __('panel/media.drag_here') }}<em>{{ __('panel/media.click_upload') }}</em></div>
        <template #tip>
        <div class="el-upload__tip">{{ __('panel/media.upload_tip') }}</div>
        <div class="el-upload__tip">{{ __('panel/media.upload_max_size') }}: {{ $uploadMaxFileSize ?? '-' }}</div>
        <div class="el-upload__tip">{{ __('panel/media.post_max_size') }}: {{ $postMaxSize ?? '-' }}</div>
        </template>
      </el-upload>
    </el-dialog>
    ...
    <el-dialog title="{{ __('panel/media.rename') }}" v-model="renameDialog.visible" custom-class="rename-dialog" width="500px">
      ...
      <template #footer>
        <el-button @click="renameDialog.visible = false">{{ __('panel/media.cancel_btn') }}</el-button>
        <el-button type="primary" @click="submitRename">{{ __('panel/media.ok_btn') }}</el-button>
      </template>
    </el-dialog>

    <!-- Move file dialog -->
    <el-dialog title="{{ __('panel/media.move_to') }}" v-model="moveDialog.visible" width="400px">
      ...
      <template #footer>
        <el-button @click="moveDialog.visible = false">{{ __('panel/media.cancel_btn') }}</el-button>
        <el-button type="primary" @click="submitMove">{{ __('panel/media.ok_btn') }}</el-button>
      </template>
    </el-dialog>
    ...
    <div class="file-card-context-menu" v-show="contextMenu.visible" :style="contextMenu.style">
      <ul>
        ...
        <li @click="copyFile"><el-icon><component :is="'CopyDocument'"></component></el-icon> {{ __('panel/media.copy_to') }}</li>
      </ul>
    </div>

    <!-- Copy file dialog -->
    <el-dialog title="{{ __('panel/media.copy_to') }}" v-model="copyDialog.visible" width="400px">
      ...
      <template #footer>
        <el-button @click="copyDialog.visible = false">{{ __('panel/media.cancel_btn') }}</el-button>
        <el-button type="primary" @click="submitCopy">{{ __('panel/media.ok_btn') }}</el-button>
      </template>
    </el-dialog>
    ...
    <div v-if="folderContextMenu.visible" class="file-card-context-menu"
      ...
      <ul>
        <li @click="createFolderUnder(folderContextMenu.folder)">
          <el-icon><component :is="'FolderAdd'"></component></el-icon> {{ __('panel/media.create_folder') }}
        </li>
        ...
        <li @click="moveFolder">
          <el-icon><component :is="'Right'"></component></el-icon> {{ __('panel/media.move_to') }}
        </li>
    ...
    <el-dialog title="{{ __('panel/media.rename_folder') }}" v-model="folderRenameDialog.visible" width="400px">
      ...
      <template #footer>
        <el-button @click="folderRenameDialog.visible = false">{{ __('panel/media.cancel_btn') }}</el-button>
        <el-button type="primary" @click="submitFolderRename">{{ __('panel/media.ok_btn') }}</el-button>
      </template>
    </el-dialog>

    <!-- Move folder dialog -->
    <el-dialog title="{{ __('panel/media.move_folder') }}" v-model="folderMoveDialog.visible" width="400px">
      ...
      <template #footer>
        <el-button @click="folderMoveDialog.visible = false">{{ __('panel/media.cancel_btn') }}</el-button>
        <el-button type="primary" @click="submitFolderMove">{{ __('panel/media.ok_btn') }}</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="storageConfigDialog.visible" width="560px" custom-class="storage-config-dialog" :close-on-click-modal="false">
      ...
      <div class="storage-dialog-header">
        <div class="storage-dialog-title">
          <el-icon><component :is="'Setting'"></component></el-icon>
          <span>{{ __('panel/media.storage_config') }}</span>
        </div>
        <p class="storage-dialog-desc">{{ __('panel/media.storage_config_desc') }}</p>
      </div>
      </template>
      <div class="storage-card-grid">
        ...
        >
          ...
          <div class="storage-card-current" v-if="option.value === storageConfigDialog.currentDriver && storageConfigDialog.driver !== option.value">
            {{ __('panel/media.storage_current') }}
          </div>
        </div>
      </div>
      <template #footer>
      <span class="storage-dialog-footer">
        <a href="{{ route('panel.settings.index') }}?tab=tab-setting-storage" target="_blank" class="storage-settings-link">
          <el-icon><component :is="'Link'"></component></el-icon> {{ __('panel/media.storage_settings_link') }}
        </a>
        <el-button @click="storageConfigDialog.visible = false">{{ __('panel/media.cancel_btn') }}</el-button>
        <el-button type="primary" @click="saveStorageConfig">{{ __('panel/media.ok_btn') }}</el-button>
      </span>
      </template>
    </el-dialog>
    ...
    <!-- Media Detail Drawer -->
    <el-drawer v-model="mediaDetail.visible" title="{{ __('panel/media.detail') }}" direction="rtl" size="500px" :destroy-on-close="true">
      <div v-loading="mediaDetail.loading" class="media-detail-body">
        <template v-if="mediaDetail.data">
          <div class="media-detail-preview" v-if="mediaDetail.data.url">
            <img :src="mediaDetail.data.url" :alt="mediaDetail.data.alt || mediaDetail.data.original_name || ''">
          </div>

          <el-descriptions :column="1" border size="small" class="media-detail-desc">
            <el-descriptions-item label="{{ __('panel/media.original_name') }}">
              @{{ mediaDetail.data.original_name || '-' }}
            </el-descriptions-item>
            <el-descriptions-item label="{{ __('panel/media.storage_disk') }}">
              <el-tag size="small" type="info">@{{ mediaDetail.data.disk }}</el-tag>
              <el-tag v-if="mediaDetail.data.source" size="small" type="warning" style="margin-left: 4px;">@{{ mediaDetail.data.source }}</el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="{{ __('panel/media.mime_type') }}">
              @{{ mediaDetail.data.mime || '-' }}
            </el-descriptions-item>
            <el-descriptions-item label="{{ __('panel/media.file_size') }}">
              @{{ mediaDetail.data.size_readable }} <span class="text-muted small">(@{{ mediaDetail.data.size }} B)</span>
            </el-descriptions-item>
            <el-descriptions-item label="{{ __('panel/media.dimensions') }}" v-if="mediaDetail.data.width || mediaDetail.data.height">
              @{{ mediaDetail.data.width }} × @{{ mediaDetail.data.height }} px
            </el-descriptions-item>
            <el-descriptions-item label="{{ __('panel/media.checksum') }}">
              <code class="media-detail-checksum">@{{ mediaDetail.data.checksum }}</code>
            </el-descriptions-item>
            <el-descriptions-item label="{{ __('panel/media.storage_path') }}">
              <code class="media-detail-path">@{{ mediaDetail.data.storage_key }}</code>
            </el-descriptions-item>
            <el-descriptions-item label="{{ __('panel/media.uploaded_at') }}">
              @{{ mediaDetail.data.created_at }}
            </el-descriptions-item>
            <el-descriptions-item label="{{ __('panel/media.used_by') }}">
              <template v-if="mediaDetail.data.total_usage > 0">
                <el-tag v-for="(count, table) in mediaDetail.data.usage" :key="table" size="small" type="success" style="margin-right: 4px;">
                  @{{ table }}: @{{ count }}
                </el-tag>
              </template>
          ...
          <div class="media-detail-alt-section">
            ...
            <el-input
              type="textarea"
              :rows="3"
              v-model="mediaDetail.altDraft"
              :placeholder="'{{ __('panel/media.alt_text_placeholder') }}'"
            ></el-input>
            <div class="media-detail-alt-actions">
              <el-button @click="mediaDetail.visible = false">{{ __('panel/media.cancel') }}</el-button>
              <el-button type="primary" @click="saveMediaAlt" :loading="mediaDetail.saving">{{ __('panel/media.save') }}</el-button>
            </div>
          </div>
        </template>
      </div>
