...
use InnoShop\Panel\Controllers\CatalogController;
...
class CatalogControllerTest extends TestCase
{
    ...
    {
        parent::setUp();
        $this->reflection = new ReflectionClass(CatalogController::class);
    }

    #[Test]
    public function test_controller_exists(): void
    {
        $this->assertTrue(class_exists(CatalogController::class));
    }
    ...
    #[Test]
    public function test_store_method_accepts_catalog_request(): void
    {
        $method     = $this->reflection->getMethod('store');
        $parameters = $method->getParameters();
        $this->assertCount(1, $parameters);
    ...
    #[Test]
    public function test_edit_method_accepts_catalog_parameter(): void
    {
        $method     = $this->reflection->getMethod('edit');
        $parameters = $method->getParameters();
        $this->assertCount(1, $parameters);
        $this->assertEquals('catalog', $parameters[0]->getName());
    }

    #[Test]
    public function test_update_method_accepts_request_and_catalog(): void
    {
        $method     = $this->reflection->getMethod('update');
        $parameters = $method->getParameters();
        $this->assertCount(2, $parameters);
        $this->assertEquals('request', $parameters[0]->getName());
        $this->assertEquals('catalog', $parameters[1]->getName());
    }

    #[Test]
    public function test_destroy_method_accepts_catalog_parameter(): void
    {
        $method     = $this->reflection->getMethod('destroy');
        $parameters = $method->getParameters();
        $this->assertCount(1, $parameters);
