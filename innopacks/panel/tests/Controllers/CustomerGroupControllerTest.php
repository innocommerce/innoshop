...
use InnoShop\Panel\Controllers\CustomerGroupController;
...
class CustomerGroupControllerTest extends TestCase
{
    ...
    {
        parent::setUp();
        $this->reflection = new ReflectionClass(CustomerGroupController::class);
    }

    #[Test]
    public function test_controller_exists(): void
    {
        $this->assertTrue(class_exists(CustomerGroupController::class));
    }
    ...
    #[Test]
    public function test_store_method_accepts_customer_group_request(): void
    {
        $method     = $this->reflection->getMethod('store');
        $parameters = $method->getParameters();
        $this->assertCount(1, $parameters);
    ...
    #[Test]
    public function test_edit_method_accepts_customer_group_parameter(): void
    {
        $method     = $this->reflection->getMethod('edit');
        $parameters = $method->getParameters();
        $this->assertCount(1, $parameters);
        $this->assertEquals('customerGroup', $parameters[0]->getName());
    }

    #[Test]
    public function test_update_method_accepts_request_and_customer_group(): void
    {
        $method     = $this->reflection->getMethod('update');
        $parameters = $method->getParameters();
        $this->assertCount(2, $parameters);
        $this->assertEquals('request', $parameters[0]->getName());
        $this->assertEquals('customerGroup', $parameters[1]->getName());
    }

    #[Test]
    public function test_destroy_method_accepts_customer_group_parameter(): void
    {
        $method     = $this->reflection->getMethod('destroy');
        $parameters = $method->getParameters();
        $this->assertCount(1, $parameters);
