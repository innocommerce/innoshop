...
use InnoShop\Panel\Controllers\WeightClassController;
use PHPUnit\Framework\Attributes\Test;
use PHPUnit\Framework\TestCase;
use ReflectionClass;

class WeightClassControllerTest extends TestCase
{
    ...
    {
        parent::setUp();
        $this->reflection = new ReflectionClass(WeightClassController::class);
    }

    #[Test]
    public function test_controller_exists(): void
    {
        $this->assertTrue(class_exists(WeightClassController::class));
    }
    ...
    #[Test]
    public function test_store_method_accepts_weight_class_request(): void
    {
        $method     = $this->reflection->getMethod('store');
        $parameters = $method->getParameters();
        $this->assertCount(1, $parameters);
    ...
    #[Test]
    public function test_edit_method_accepts_weight_class_parameter(): void
    {
        $method     = $this->reflection->getMethod('edit');
        $parameters = $method->getParameters();
        $this->assertCount(1, $parameters);
        $this->assertEquals('weight_class', $parameters[0]->getName());
    }

    #[Test]
    public function test_update_method_accepts_request_and_weight_class(): void
    {
        $method     = $this->reflection->getMethod('update');
        $parameters = $method->getParameters();
        $this->assertCount(2, $parameters);
        $this->assertEquals('request', $parameters[0]->getName());
        $this->assertEquals('weight_class', $parameters[1]->getName());
    }

    #[Test]
    public function test_destroy_method_accepts_weight_class_parameter(): void
    {
        $method     = $this->reflection->getMethod('destroy');
        $parameters = $method->getParameters();
        $this->assertCount(1, $parameters);
