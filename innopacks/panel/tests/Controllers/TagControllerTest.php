...
use InnoShop\Panel\Controllers\TagController;
...
class TagControllerTest extends TestCase
{
    ...
    {
        parent::setUp();
        $this->reflection = new ReflectionClass(TagController::class);
    }

    #[Test]
    public function test_controller_exists(): void
    {
        $this->assertTrue(class_exists(TagController::class));
    }
    ...
    #[Test]
    public function test_store_method_accepts_tag_request(): void
    {
        $method     = $this->reflection->getMethod('store');
        $parameters = $method->getParameters();
        $this->assertCount(1, $parameters);
    ...
    #[Test]
    public function test_edit_method_accepts_tag_parameter(): void
    {
        $method     = $this->reflection->getMethod('edit');
        $parameters = $method->getParameters();
        $this->assertCount(1, $parameters);
        $this->assertEquals('tag', $parameters[0]->getName());
    }

    #[Test]
    public function test_update_method_accepts_request_and_tag(): void
    {
        $method     = $this->reflection->getMethod('update');
        $parameters = $method->getParameters();
        $this->assertCount(2, $parameters);
        $this->assertEquals('request', $parameters[0]->getName());
        $this->assertEquals('tag', $parameters[1]->getName());
    }

    #[Test]
    public function test_destroy_method_accepts_tag_parameter(): void
    {
        $method     = $this->reflection->getMethod('destroy');
        $parameters = $method->getParameters();
        $this->assertCount(1, $parameters);
    ...
    #[Test]
    public function test_create_data_structure(): void
    {
        $data = [
            'tag' => null,
        ];
