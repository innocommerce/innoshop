  ...
  @foreach($menuLinks as $index => $menuLink)
    <div class="accordion-item">
      @if(!$menuLink['has_children'])
        <a class="accordion-button {{ $menuLink['active'] ? 'active' : '' }}" href="{{ $menuLink['url'] }}">
          <span class="icon"><i class="bi {{ $menuLink['icon'] ?? 'bi-house' }}"></i></span> {{ $menuLink['title'] }}
        </a>
      @else
        <h2 class="accordion-header">
          <button class="accordion-button {{ $menuLink['active'] ? '' : '' }}" type="button"
        ...
        <div id="flush-collapseOne-{{ $index }}"
             class="accordion-collapse collapse {{ $menuLink['active'] ? 'show' : 'show' }}"
