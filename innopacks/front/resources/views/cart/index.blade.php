              ...
              <tr>
                <th scope="col">
                  ...
                  <input class="form-check-input product-all-check" type="checkbox"
                    @if (count($selectedItems) == count($list)) checked @endif>
                </th>
              ...
              @foreach ($list as $product)
                <tr data-id="{{ $product['id'] }}">
                  <td class="td-product-check">
                    <input class="form-check-input product-item-check" value="{{ $product['id'] }}" type="checkbox"
                  ...
                  <td class="td-delete">
                    <div class="delete-cart text-danger fs-5 cursor-pointer"><i class="bi bi-x-circle-fill"></i></div>
                  </td>
                </tr>
              @endforeach
            </tbody>
...
@push('footer')
  <script>
    ...
    function updateSelectedCarts(selected, ids) {
      axios.post(`${urls.cart_add}/${selected ? 'select' : 'unselect'}`, {
        cart_ids: ids
      }).then(function(res) {
        if (res.success) {
          inno.msg(res.message)
