...
class HunyuanService implements AIServiceInterface
{
    private $client;
    ...
    public function __construct(array $config)
    {
        ...
        $this->client = OpenAI::factory()
            ...
            ->withBaseUri($config['base_url'] ?? 'https://hunyuan.cloud.tencent.com/v1')
    ...
    public function generate(string $prompt, array $options = []): string
    {
        ...
        $response = $this->client->chat()->create([
            'model'       => $options['model'] ?? $this->config['model'] ?? 'hunyuan-standard',
            'messages'    => $messages,
