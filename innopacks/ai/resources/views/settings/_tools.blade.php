...
<div class="tab-pane fade show active" id="ai-tools" role="tabpanel" aria-labelledby="ai-tools-tab">
  ...
  <div class="card mb-4">
    <div class="card-header d-flex justify-content-between align-items-center">
      <h5 class="card-title mb-0">{{ __('ai::setting_ai.provider_management') }}</h5>
      <button type="button" class="btn btn-sm btn-outline-primary" onclick="aiAddProvider()">
        <i class="bi bi-plus-lg me-1"></i>{{ __('ai::setting_ai.add_provider') }}
      </button>
    </div>
    <div class="card-body">
      <p class="text-muted small mb-3">{{ __('ai::setting_ai.provider_management_desc') }}</p>
  ...
  <div class="card mb-4">
    <div class="card-header">
      <h5 class="card-title mb-0">{{ __('ai::setting_ai.capability_selection') }}</h5>
    </div>
    <div class="card-body">
      <p class="text-muted small mb-3">{{ __('ai::setting_ai.capability_selection_desc') }}</p>
      <div class="row">
        <div class="col-lg-6">
          <x-common-form-select title="{{ __('ai::setting_ai.text_generation_provider') }}" name="ai_text_provider"
        ...
        <div class="col-lg-6">
          <x-common-form-input title="{{ __('ai::setting_ai.text_model_override') }}"
                             name="ai_text_model"
                             value="{{ old('ai_text_model', system_setting('ai_text_model')) }}"
                             placeholder="{{ __('ai::setting_ai.model_override_placeholder') }}" />
        </div>
      </div>
      <div class="row mt-3">
        <div class="col-lg-6">
          <x-common-form-select title="{{ __('ai::setting_ai.image_generation_provider') }}" name="ai_image_provider"
                              :options="$ai_models ?? []" key="code" label="name" :emptyOption="true"
                              value="{{ old('ai_image_provider', system_setting('ai_image_provider')) }}" />
        </div>
        <div class="col-lg-6">
          <x-common-form-input title="{{ __('ai::setting_ai.image_model_override') }}"
                             name="ai_image_model"
...
<script>
// Provider Management
const aiPresets = {!! json_encode(($ai_presets ?? []) ?: app(\InnoShop\AI\Services\ProviderRegistry::class)->getPresets()) !!};
...
function aiRenderProviders() {
  ...
  if (!hasUser && !hasPlugin) {
    list.innerHTML = '<div class="text-muted text-center py-3">{{ __("ai::setting_ai.no_providers") }}</div>';
    return;
  }
  ...
  if (hasPlugin) {
    html += '<div class="mb-3"><small class="text-muted fw-bold">{{ __("ai::setting_ai.plugin_providers") }}</small></div>';
    aiPluginProviders.forEach(function(p) {
      ...
      html += '<span class="badge bg-info text-dark">{{ __("ai::setting_ai.from_plugin") }}</span>';
      html += '</div></div>';
      html += '<div class="card-body"><div class="row">';
      html += '<div class="col-lg-6 mb-2"><label class="form-label small">{{ __("ai::setting_ai.provider_base_url") }}</label>';
      html += '<input type="text" class="form-control form-control-sm" value="' + (p.base_url || '') + '" disabled /></div>';
      html += '<div class="col-lg-6 mb-2"><label class="form-label small">{{ __("ai::setting_ai.text_model") }}</label>';
      html += '<input type="text" class="form-control form-control-sm" value="' + (p.models?.text || '') + '" disabled /></div>';
      if (p.models?.image) {
        html += '<div class="col-lg-6 mb-2"><label class="form-label small">{{ __("ai::setting_ai.image_model") }}</label>';
  ...
  if (hasUser) {
    if (hasPlugin) {
      html += '<div class="mb-3 mt-4"><small class="text-muted fw-bold">{{ __("ai::setting_ai.user_providers") }}</small></div>';
    }
    aiProviders.forEach(function(p, idx) {
      ...
      html += '<label class="form-label small">{{ __("ai::setting_ai.provider_api_key") }}</label>';
      ...
      // Base URL
      html += '<div class="col-lg-6 mb-2">';
      html += '<label class="form-label small">{{ __("ai::setting_ai.provider_base_url") }}</label>';
      ...
      html += '<div class="row">';
      html += '<div class="col-lg-6 mb-2">';
      html += '<label class="form-label small">{{ __("ai::setting_ai.text_model") }}</label>';
      ...
      html += '<button type="button" class="btn btn-outline-secondary" onclick="aiFetchModels(' + idx + ', \'text\', this)" title="{{ __("ai::setting_ai.fetch_models") }}"><i class="bi bi-arrow-down-circle"></i></button>';
      html += '</div>';
      html += '</div>';
      html += '<div class="col-lg-6 mb-2">';
      html += '<label class="form-label small">{{ __("ai::setting_ai.image_model") }}</label>';
      html += '<div class="input-group input-group-sm">';
      ...
      html += '<button type="button" class="btn btn-outline-secondary" onclick="aiFetchModels(' + idx + ', \'image\', this)" title="{{ __("ai::setting_ai.fetch_models") }}"><i class="bi bi-arrow-down-circle"></i></button>';
      html += '</div>';
      html += '</div>';
      html += '</div>';
...
function aiFetchModels(idx, type, btn) {
  ...
  if (!provider) return;
  if (!provider.api_key) {
    alert('{{ __("ai::setting_ai.fetch_models_need_key") }}');
    return;
  }
  if (!provider.base_url) {
    alert('{{ __("ai::setting_ai.fetch_models_need_base_url") }}');
    return;
  }
  ...
  axios.post('{{ panel_route("content_ai.list_models") }}', {
    ...
    if (models.length === 0) {
      alert('{{ __("ai::setting_ai.fetch_models_empty") }}');
      return;
    }
    ...
    alert('{{ __("ai::setting_ai.fetch_models_failed") }}' + (err.response?.data?.message || err.message));
  }).finally(() => {
...
function aiRemoveProvider(idx) {
  if (!confirm('{{ __("ai::setting_ai.confirm_remove_provider") }}')) return;
...
function aiAddProvider() {
  ...
  html += '<div class="modal-header"><h5 class="modal-title">{{ __("ai::setting_ai.add_provider") }}</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>';
  ...
  html += '<strong class="small">{{ __("ai::setting_ai.preset_custom") }}</strong>';
  html += '</div></div></div>';
  html += '<div class="col-6 col-md-4">';
  html += '<div class="card text-center h-100 provider-card" style="cursor:pointer;" onclick="aiSelectPreset(\'custom_anthropic\')">';
  html += '<div class="card-body d-flex flex-column align-items-center justify-content-center py-3">';
  html += '<div class="mb-2 rounded bg-light d-flex align-items-center justify-content-center" style="width:36px;height:36px;"><i class="bi bi-plus-lg text-secondary"></i></div>';
  html += '<strong class="small">{{ __("ai::setting_ai.preset_custom_anthropic") }}</strong>';
  html += '</div></div></div>';
