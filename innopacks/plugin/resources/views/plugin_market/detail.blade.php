        ...
        <div class="col-12">
          ...
          @elseif(isset($product) && is_array($product) && isset($product['success']) && $product['success'])
            ...
            <div class="card border rounded-3 shadow-sm mb-4" style="border-color: #e9ecef !important; border-width: 1.5px;">
              <div class="position-absolute top-0 start-0 w-100" style="height: 4px; background: linear-gradient(90deg, #0d6efd 0%, #0dcaf0 100%);"></div>
              <div class="card-body p-4">
                <div class="row g-4">
                  <div class="col-md-4 text-center">
                    <img src="{{ $product['image_big'] ?? $product['image_small'] ?? '' }}" 
                         class="img-fluid rounded-3 shadow-sm" 
                         style="max-width: 280px; width: 100%; border: 2px solid #e9ecef; background: #f8f9fa; padding: 20px;"
                         alt="{{ $product['name'] }}">
                  </div>
                  <div class="col-md-8">
                    ...
                    @if($product['downloadable'] ?? false)
                      <button class="btn btn-primary btn-lg px-5" type="button" id="btnDownload" data-product-type="plugin">
    ...
    <script>
        $(function () {
            ...
            $('#btnDownload').click(function (){
                axios({
                    method: "get",
                    url: '{{ panel_route('marketplaces.download', ['id' => $product['product_id'] ?? 0, 'type' => 'plugin']) }}',
