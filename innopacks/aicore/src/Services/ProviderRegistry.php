...
namespace InnoShop\Aicore\Services;
...
class ProviderRegistry
{
    ...
    /**
     * Single source of truth for the default text provider code.
     * Preference: explicit ai_text_provider setting → legacy ai_model → 'glm'.
     */
    public function getDefaultTextProvider(): string
    {
        return (string) (system_setting('ai_text_provider')
     ...
     * Note: 'ai.*' is the namespace Laravel AI SDK reads (see vendor/laravel/ai AiManager::getInstanceConfig).
     * Earlier this wrote to 'aicore.*' which laravel/ai never read, so every custom provider silently fell
     * through to the openai default driver and hit api.openai.com/v1/responses.
     */
    public function buildLaravelAiConfig(): void
    {
        ...
        $textProvider = $this->getDefaultTextProvider();
