...
InnoShop 是一个基于 Laravel 13 的开源电子商务系统，支持多语言、多货币，集成了多种 AI 模型并原生支持 MCP（Model Context Protocol）协议，实现智能自动化。它还具有插件机制和主题模板开发功能，以增强用户体验和系统的可扩展性。
...
`create-project` 命令会自动完成：
1. 安装 PHP 依赖
2. 生成应用密钥
3. 执行 `npm install` 与 `npm run build` 编译前端资源
...
- 基于最新技术, 深度集成 AI 并原生支持 MCP（Model Context Protocol）, 支持多语言和多货币等特性。
