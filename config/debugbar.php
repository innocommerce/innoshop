...
return [
    ...
    /*
     ...
     |
     | InnoShop override: when DEBUGBAR_ENABLED env is unset, fall back to the
     | backend `system.debug` setting so the panel toggle controls the bar even
     | though Debugbar v4 gates on app.debug inside its boot().
     |
     */

    'enabled' => function () {
        $env = env('DEBUGBAR_ENABLED');
        if ($env !== null) {
            return filter_var($env, FILTER_VALIDATE_BOOLEAN);
        }
        if (! function_exists('installed') || ! installed()) {
            return false;
        }
        if (! config('inno')) {
            load_settings();
        ...
        return (bool) system_setting('debug', false);
    },
